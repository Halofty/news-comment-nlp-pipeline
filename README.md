...
| LLM Batch·Langfuse | 설계·도입 검토 | 구현 전 |
| Airflow orchestration | 계획 | 구현 전 |

전체 자동 테스트는 현재 43개가 통과합니다. PostgreSQL 적재는 1,000건 규모의 Driver chunk upsert 방식이며, 대규모 확장에서는 JDBC staging 또는 bulk load로 교체할 예정입니다.
