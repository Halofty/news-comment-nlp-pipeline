...
이는 구조 검증만으로 의미 품질을 보장할 수 없다는 증거다. 월간 요청 전 다음 후처리를
적용한다.
...
- 22~31일 완료 후 같은 다운로드·검증 수행
- 31개 `event_id`의 날짜 연속성, 유일성, 누락 여부 확인
- 위 의미 품질 필터를 적용해 월간 입력 생성
- 일별 31개 응답으로 1월 통합 Batch 1건 제출
- 최종 실제 비용과 Langfuse 화면을 과제 보고서에 반영
